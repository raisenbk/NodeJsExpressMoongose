...
const getCustomers = async (req, res, next) => {
  try {
    ...
    // 3. Sorting
    // sorting ASCENDING = name, kalau DESCENDING = -name (pakai minus atau strip)
    if(req.query.sort) {
    ...
    // 4. Field limiting => Menampilkan hanya name dan age
    if (req.query.fields) {
    ...
    if(req.query.page){
      ...
      if (skip > newCustomers) throw new Error("Page does not exist!")
    }
